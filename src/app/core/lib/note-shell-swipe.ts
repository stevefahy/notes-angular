import type { NoteShellLayout } from './note-shell-dom';

export type NoteShellSwipeHandlers = {
  onSwipeToView: () => void;
  onSwipeToEdit: () => void;
};

/**
 * Horizontal swipe to mirror footer Edit/View toggle. Disabled when layout === "split".
 * Port of notes-nextjs-public/lib/useNoteShellSwipeNavigation.ts
 */
export function attachNoteShellSwipeNavigation(
  el: HTMLElement,
  getLayout: () => NoteShellLayout,
  handlers: NoteShellSwipeHandlers,
): () => void {
  ...
  const trySwipe = (
    ...
    if (current === 'edit' && dx < 0) {
      handlers.onSwipeToView();
    } else if (current === 'view' && dx > 0) {
      handlers.onSwipeToEdit();
    }
  };
